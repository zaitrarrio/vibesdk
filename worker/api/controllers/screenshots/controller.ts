...
export class ScreenshotsController extends BaseController {
    ...
    async serveScreenshot(
        ...
        try {
            ...
            if (!sessionId || !file) {
                return this.createErrorResponse('Missing path parameters', 400);
            }

            // Validate and sanitize path parameters
            if (!this.isValidSessionId(sessionId)) {
                return this.createErrorResponse('Invalid session id', 400);
            }

            const validatedFile = this.validateFileName(file);
            if (!validatedFile) {
                return this.createErrorResponse('Invalid file name', 400);
            }

            const key = `screenshots/${sessionId}/${validatedFile}`;
            const obj = await env.TEMPLATES_BUCKET.get(key);
            if (!obj || !obj.body) {
                return this.createErrorResponse('Screenshot not found', 404);
            }

            const contentType = obj.httpMetadata?.contentType || this.getMimeByExtension(validatedFile) || 'image/png';
    ...
    private validateFileName(file: string): string | null {
    ...
    private getMimeByExtension(file: string): string | undefined {
